...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        try:
            btn = [[
                InlineKeyboardButton(text=f"🔍 𝚂𝚎𝚊𝚛𝚌𝚑 𝙷𝚎𝚛𝚎 🔎", url="https://t.me/Mannu_Production")
            ],[
                InlineKeyboardButton(text=f"🐞 ʀᴇᴘᴏʀᴛ ɪssᴜᴇ 🐞", url=f"https://t.me/MS_Dhaliwal")
            ]]
    ...
    if query.data.startswith("file"):        
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer(f"Hey, {query.from_user.first_name}! 😉 𝙸 𝙻𝚒𝚔𝚎 𝚈𝚘𝚞𝚛 𝚂𝚖𝚊𝚛𝚝𝚗𝚎𝚜𝚜, 𝙱𝚞𝚝 𝙳𝚘𝚗'𝚝 𝙱𝚎 𝙾𝚟𝚎𝚛𝚜𝚖𝚊𝚛𝚝 𝙾𝚔𝚊𝚢 😉", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        buttons = [[
            ...
            InlineKeyboardButton("📢 𝚄𝙿𝙳𝙰𝚃𝙴𝚂 📢", url="https://t.me/Mannu_Production")
            ],[
            InlineKeyboardButton("♻️ 𝙷𝙴𝙻𝙿 ♻️", callback_data="help"),
            InlineKeyboardButton("💫 𝙰𝙱𝙾𝚄𝚃 💫", callback_data="about")
            ],[
            InlineKeyboardButton("🎬 𝙼𝙾𝚅𝙸𝙴𝚂 𝙶𝚁𝙾𝚄𝙿 🎬", url=f"https://t.me/Mannu_Production")
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
