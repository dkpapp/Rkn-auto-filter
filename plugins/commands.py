...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton("➕️ 𝙰𝙳𝙳 𝙼𝙴 𝚃𝙾 𝚈𝙾𝚄𝚁 𝙶𝚁𝙾𝚄𝙿 ➕️", url=f"http://t.me/RKN_SERIES_SEARCH_BOT?startgroup=true")
            ],[
            ...
            InlineKeyboardButton("🎬 𝙰𝙻𝙻 𝚂𝙴𝚁𝙸𝙴𝚂 𝙶𝚁𝙾𝚄𝙿 🎬", url=f"https://t.me/CG_TVSHOW_SERIES_GROUP")
         ]]
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        buttons = [[
            ...
            InlineKeyboardButton("🎬 𝙼𝙾𝚅𝙸𝙴 𝙶𝚁𝙾𝚄𝙿 🎬", url=f"https://t.me/CG_TVSHOW_SERIES_GROUP")
         ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await message.reply_chat_action(enums.ChatAction.TYPING)
        m=await message.reply_sticker("CAACAgUAAxkBAAIFNGJSlfOErbkSeLt9SnOniU-58UUBAAKaAAPIlGQULGXh4VzvJWoeBA")
